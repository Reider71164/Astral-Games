...
function createBareServer(directory, init = {}) {
    ...
    init.logErrors = init.logErrors !== undefined ? init.logErrors : false;
    const cleanup = [];
    ...
    if (init.blockLocal ?? true) {
        init.filterRemote = init.filterRemote !== undefined ? init.filterRemote : (url) => {
        ...
        init.lookup = init.lookup || ((hostname, options, callback) => {
            node_dns_1.lookup(hostname, options, (err, address, family) => {
                if (address &&
                    toAddressArray(address, family).some(({ address }) => ipaddr_js_1.parse(address).range() !== 'unicast')) {
    ...
    init.legacySupport = init.legacySupport !== undefined ? init.legacySupport : true;
